...
export const roles = {
  myNewRole: {
    ...
    passives: {
      onDeath: (player, killer, game) => { /* triggered when killed */ },
    },
  },
};
```
...
export const events = {
  myEvent: {
    ...
    participants: (game) => game.getAlivePlayers().filter(p => p.role.id === 'myRole'),
    validTargets: (actor, game) => game.getAlivePlayers().filter(p => p.id !== actor.id),
